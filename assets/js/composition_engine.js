...
export class CompositionEngine {
  ...
  evaluateCandidates(candidates, scores) {
    return candidates.map((candidate, idx) => {
      ...
      const emphasisedBaseScore = isModelInference
        ? clamp(
            baseScore * 1.12 +
              Math.max(0, compositionScore - 0.6) * 0.2 +
              Math.max(0, aestheticScore - 0.6) * 0.16,
            0,
            1
          )
        : baseScore;
      ...
      const rotationPenalty = rotationDegrees
        ...
        : 0;
      const stabilityBase = clamp(1 - (rotationPenalty + balancePenalty), 0.4, 1);
      const stabilityGuard = isModelInference
        ? clamp(stabilityBase + horizonConfidence * 0.1, 0.55, 1)
        : stabilityBase;
      const overallScore = clamp(emphasisedBaseScore * stabilityGuard, 0, 1);
